...
if __name__ == '__main__':
    ...
    for i, in engine.execute(empty).fetchall():
        ...
        query = sqlalchemy.select([
            ...
            func.min(t_dist.c.distance + t_dist1.c.distance),
            20]
        ).select_from(
            t_dist.join(
                t_dist1,
        ...
        shortcuts = t_dist.insert().from_select(
            ...
            query)
        engine.execute(shortcuts)
        engine.execute(t_eco.delete(whereclause=t_eco.c.hexbin == i))
        engine.execute(t_dist.delete(whereclause=t_dist.c.hexbin1 == i))
        engine.execute(t_dist.delete(whereclause=t_dist.c.hexbin2 == i))
        engine.execute(t_hex.delete(whereclause=t_hex.c.hexbin == i))
