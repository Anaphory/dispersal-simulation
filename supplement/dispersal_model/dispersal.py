...
@cython.ccall
@cython.exceptval(check=False)
def cooperate(families_in_this_location: list) -> tuple:
    ...
    for family in families_in_this_location:
        ...
        for group in cooperative_groups:
            ...
            # of that family.
            cooperative_groups.append([family])
...
def initialization() -> State:
    ...
    while new_patches:
        ...
        if p.resources > 1:
            ...
            continue
        for q in hexgrid.neighbors_within_distance(next, meters(52413)):
            if q in patches:
                continue
            if q in new_patches:
                continue
...
def decide_on_moving(
        ...
        avoid_stay: int = 0) -> cython.ulonglong:  # hexgrid.Index
    ...
    """
    try:
        target, patch, cooperators, competitors = known_destinations.pop(0)
    except IndexError:
...
def test_decide_on_moving_is_unbiased() -> Counter[hexgrid.Index]:
    ...
    """
    family = Family(descendence='', culture=0, effective_size=1, location_history=[])
    current_patch = Patch(resources=1)
...
def resources_from_patch(
        ...
        labor: int,
        others_labor: int,
        estimate: bool = False) -> float:  # kcal
