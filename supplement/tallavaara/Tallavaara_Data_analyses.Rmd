...
```{r}
# PaleoClim.org links bioclimatic data for different time periods
URL <- "http://sdmtoolbox.org/paleoclim.org/data/chelsa_cur/CHELSA_cur_V1_2B_r2_5m.zip"
wd <- getwd()
td <- paste(wd, "data", "", sep="/")
dir.create(td)
setwd(td)
temp <- "CHELSA_cur_V1_2B_r2_5m.zip"
if (!file.exists(temp)) {
  download.file(URL, temp)
}
unzip(temp)
list.files(paste(td, "2_5min", sep="/"))
tempRast <- raster("2_5min/bio_1.tif")/10
precipRast <- raster("2_5min/bio_12.tif")/1
setwd(wd)
```
...
writeRaster(nppRast, "npp.tif", overwrite=T)
...
We downloaded mammal and bird richness data from Biodiversitymapping.org (http://biodiversitymapping.org/wordpress/index.php/home/).
```{r}
URL <- paste("http://biodiversitymapping.org/","wordpress/wp-content/uploads/2016/11/",
"biodiversitymapping_TIFFs_20Dec2016.zip", sep="")
URL <- paste("https://biodiversitymapping.org", "wp-content/uploads/2020/02",
             "BiodiversityMapping_TIFFs_2019_03d14_revised.zip", sep="/")
wd <- getwd()
setwd(td)
temp <- "BiodiversityMapping_TIFFs.zip"
if (!file.exists(temp)) {
  download.file(URL, temp)
}
unzip(temp)
...
plot(animDRast, main="animal diversity", col=viridis(100))
...
```{r}
# if (!file.exists("plantsDRast.tif")) {
  URL <- "http://ecotope.org/files/ellis_2012/ellis_2012_shapefile.zip"
  wd <- getwd()
  setwd(td)
  temp <- "ellis_2012_shapefile.zip"
  if (!file.exists(temp)) {
    download.file(URL, temp)
  }
  unzip(temp)
  dsn <- "ellis_2012_l8_dataset_2012_01_17.shp"
  plants <- readOGR(dsn=dsn, layer="ellis_2012_l8_dataset_2012_01_17")
  setwd(wd)
  ...
  # variables.
  globalGrid <- as.data.frame(
    expand.grid(x=seq(-180,180, by=0.1666667),
  ...
  writeRaster(plantDRast, "plantsDRast.tif", overwrite=T)
# } else {
#  plantDRast <- raster("plantsDRast.tif")
# }
plantDRast.scal <- plantDRast/cellStats(plantDRast,'max')

plot(plantDRast, main="plant diversity", col=viridis(100))
...
pointsRast <- raster::extract(plantDRast, cbind(plants2$x,plants2$y))
...
```{r}
# Download the data.
if (!file.exists("data/DEf01f.Rdata")) {
  download.file("http://capone.mtsu.edu/eaeff/downloads/DEf01f.Rdata", "data/DEf01f.Rdata")
}
load("data/DEf01f.Rdata")
...
Comparison of the interpolated richness to the original model-based richness:
```{r}
pointsRastPatho <- raster::extract(pathoRast,cbind(pathogens2$lon,pathogens2$lat))
...
hgdata[replace_na(hgdata$densityK > 450, F),]
hgdata[replace_na(hgdata$densityK > 450, F), "densityK"] <- NA
```
For the Chumash, Kelly (2013) cites Keeley (1988), who lists the corresponding entry (Table 1) as

> 93. – Chumash – 21.60 – King 1969 (pop); Beals and Hester 1974 (area)

and this density is a vast outlier in the population densities. (The next ones
are Eastern Pomo (16.67) and Wiyot (11.0), all other densities are estimated
at less than 10/100km².) The different sources
for population count and area make it likely that this is a vast over-estimate,
stemming from different decisions what to count as Chumash between King (1969)
and Beals and Hester (1974). Because of this, we remove this entry from our
dataset.
...
```
If population is present in both sets, we used the **weighted geometric mean of the two sources**
...
in our analyses.
```{r}
hgdata$densityC <- (hgdata$densityB * (hgdata$densityK ** 3)) ** 0.25
hgdata[is.na(hgdata$densityC), "densityC"] <- hgdata[is.na(hgdata$densityC), "densityB"]
hgdata[is.na(hgdata$densityC), "densityC"] <- hgdata[is.na(hgdata$densityC), "densityK"]
...
hist(
  hgdata$densityC,
  main="Distribution of hunter-gatherer\n population density",
  nclass=31,
...
predictordata <- as.data.frame(raster::extract(predictors,coords,layer=1,nl=3))
...
```
This last (m6) will our final model for high productivity, because analysis of spatial autocorrelation suggests that all paths are significant even when spatial autocorrelation is taken into account.
```{r}
plot(m6)
```
...
## TODO: Is visreg compatible with R 4.1.0?? I get an error in the following lines.
# visreg(fit.dens.low2, xvar="npp", points=list(cex=0.8, pch=1, col="black"), xlab="",
...
library(ggplot2)
ggplot(global.preds) + geom_raster(aes(x=x, y=y, fill=npp))
```
```{r}
...
global.stack <- rasterFromXYZ(global.preds[,c("x","y","lensemble")], crs="+proj=longlat +datum=WGS84")
...
URL <- paste(
  ...
  # I had problems downloading the current version, but 2.0.0 I could get: ?version=2.0.0
  sep="/")
wd <- getwd()
setwd(td)
temp <- "ne_50m_glaciated_areas.zip"
if (!file.exists(temp)) {
  download.file(URL, temp, mode = "wb")
}
unzip(temp)
lyr <- "ne_50m_glaciated_areas"
glaciers <- readOGR(dsn = "ne_50m_glaciated_areas.shp", layer = lyr)
setwd(wd)
...
lpdensity <- global.stack

writeRaster(lpdensity, "lpdensity.tif", overwrite=T)
```

```{r}
library(DBI)
con <- dbConnect(RSQLite::SQLite(), "../distances/with-eco.sqlite")

# res <- dbExecute(con, "ALTER TABLE nodes ADD COLUMN lpdensity REAL")

res <- dbSendQuery(con, "SELECT node_id, latitude, longitude FROM nodes WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND h3latitude IS NOT NULL AND h3longitude IS NOT NULL")
coords <- dbFetch(res)
coords <- SpatialPointsDataFrame(
  coords = coords[,c("longitude", "latitude")],
                                 data=coords, proj4string = CRS("+proj=longlat +datum=WGS84"))

coords$popdensity <- raster::extract(lpdensity, coords, method="bilinear", na.rm=T)

coords[is.na(coords$popdensity),"popdensity"] <- krige(
  popdensity~1, coords[!is.na(coords$popdensity),], coords[is.na(coords$popdensity),]
)$var1.pred

coords$popdensity <- exp(coords$popdensity)

dbWriteTable(con, "hex", as.data.frame(coords),
             overwrite=T)

ggplot(as.data.frame(coords)) +
  geom_point(aes(x=longitude, y=latitude, colour=popdensity), size=0.1) + 
