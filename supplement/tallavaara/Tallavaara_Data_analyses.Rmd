...
for (j in 1:nrow(data)) {
  ...
  preds.svm[j] <- predict(svm.l, newdata = test.iter)
  preds.svm
}
...
# and rasterize
ecozonRast <- rasterize(ecozon, worldRaster, field="OBJECTID")
summary(ecozonRast)
```
...
```
```{r}
names(ecozData) <- c("x", "y", "zcode", "ldensity", "nppEffectL", "bioEffectL", "patEffectL")
ecozData$zone = ecozon$BIOME_NAME[ecozData$zcode]
ecozData$name = ecozon$ECO_NAME[ecozData$zcode]
ecozData$id = ecozon$ECO_ID[ecozData$zcode]
ecozData$zone[ecozData$zcode==11] <- "Tundra"

ecozData$zcode[ecozData$zone=="Tundra"] <- 1
ecozData$zcode[ecozData$zone=="Boreal Forests/Taiga"] <- 2
ecozData$zcode[ecozData$zone=="Montane Grasslands & Shrublands"] <- 3
ecozData$zcode[ecozData$zone=="Temperate Conifer Forests"] <- 4
ecozData$zcode[ecozData$zone=="Temperate Broadleaf & Mixed Forests"] <- 5
ecozData$zcode[ecozData$zone=="Temperate Grasslands, Savannas & Shrublands"] <- 6
ecozData$zcode[ecozData$zone=="Mediterranean Forests, Woodlands & Scrub"] <- 7
ecozData$zcode[ecozData$zone=="Deserts & Xeric Shrublands"] <- 8
ecozData$zcode[ecozData$zone=="Tropical & Subtropical Grasslands, Savannas & Shrublands"] <- 9
ecozData$zcode[ecozData$zone=="Flooded Grasslands & Savannas"] <- 10
ecozData$zcode[ecozData$zone=="Mangroves"] <- 11
ecozData$zcode[ecozData$zone=="Tropical & Subtropical Moist Broadleaf Forests"] <- 12
ecozData$zcode[ecozData$zone=="Tropical & Subtropical Dry Broadleaf Forests"] <- 13
ecozData$zcode[ecozData$zone=="Tropical & Subtropical Coniferous Forests"] <- 14
...
medlwd=0.7, yaxt="n")
points(1:14, meansNPPEF$nppEffectL, col = "black", pch=19, cex=0.30)
...
ylim=c(0,1), medcol="black", whisklty=1,whiskcol=cols,staplecol=cols,
medlwd=0.7, yaxt="n")
points(1:14, meansBIOEF$bioEffectL, col = "black", pch=19, cex=0.30)
...
ylim=c(0,1), medcol="black", whisklty=1,whiskcol=cols,staplecol=cols,
medlwd=0.7, yaxt="n")
points(1:14, meansPATEF$patEffectL, col = "black", pch=19, cex=0.30)
axis(side=2, at=c(0,0.2,0.4,0.6,0.8,1), las=2, tck=-0.03)
...
points(1:14, means$ldensity, col = "black", pch=19, cex=0.30)
