...
families-own
[
  my-neighborhood       ; list of families around
  my-helpers            ; subset of my-neighborhood that will help me (with probability 95%)
  my-group              ; list of agents withins a social aggregate (i.e. families that belong to my group/component)
  group-size            ; size of the group (component) where I belong
  collected-energy      ; energy obtained during the current tick
  total-energy          ; total level of energy (including surplus)
  identity              ; 10-dimension vector with values 1 (no important) to 6 (very important)
  labor                 ; number of individuals within a household
  survival-threshold    ; 730 * labor
  technology            ; Gaussian-distributed: mean: average-technology; standard deviation: diversity
  conservation-factor   ; current value of technology divided by two
  individual-capability ; fraction of the resource that the agent is able to obtain when working on their own
  cultural-distance     ; cooperation will be posible if similarity >= cultural-distance
  cooperation?          ; true if I have been involved in a cooperation process (i.e. I have helped someone or someone has helped me)
  explored?             ; needed for detecting components
  component             ; group (i.e. component in the network) where the family belongs             
]

patches-own
[
  difficulty       ; (h)
  max-resource     ; maximum amount of resource (in hot season)
  resource         ; current amount of resource
  amount-consumed  ; amount of energy consumed during the current tick 
  exploited?       ; true during one tick after been exploited
]
...
to setup
  ...
  set weights-vector [0.5 0.25 0.12 0.06 0.03 0.015 0.0075 0.00375 0.001875 0.0009375]
  
  
  ;CREATE AND INITIALIZE FAMILIES
  
  let initial-identity n-values 10 [1 + random 5]  ;all the families start with the same identity vector
...
to update-resources-on-patches 
...
to-report calculate-individual-capability
...
to-report calculate-aggregated-capability [agents-willing-to-cooperate]  
...
to decide-whether-to-move-or-stay
  ask families
  [
...
to-report compute-consensual-identity [group]
  let consensus []
  foreach n-values 10 [?]
    [ 
